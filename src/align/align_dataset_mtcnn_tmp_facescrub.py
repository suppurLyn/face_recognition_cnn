...
def main(args):
    ...
    imgToRec = dict()
    _file = open(args.links_dir, 'r')
    line = _file.readline()
    while line:
        arr = line.split()
        imgToRec[arr[0]] = [int(x) for x in arr[1].split(',')]
        line = _file.readline()
    _file.close()
    ...
    with open(bounding_boxes_filename, "w") as text_file:
        ...
        for cls in dataset:
            ...
            for image_path in cls.image_paths:
                try:
                    bb = imgToRec[os.path.split(image_path)[-1]][0:4]
                except:
                    print('no such files')
                nrof_images_total += 1
                filename = os.path.split(image_path)[-1]
                output_filename = os.path.join(output_class_dir, str(output_label)+'.jpg')
                if not os.path.exists(output_filename):
                    try:
                        ...
                        if nrof_faces>0:
                            ...
                            rates = [overlap_ratio(x, bb) for x in det.tolist()]
                            if sum([x>0.6 for x in rates]) == 1:
...
if __name__ == '__main__':
    sys.argv = [sys.argv[0], r'/media/lyn/DISK_E/facescrub', r'/media/lyn/DISK_E/facescrub_ali_faces_160/',
                r'/media/lyn/DISK_E/bboxes.txt']
